...
const statusStyles = {
    ...
    low_stock: {
        ...
        icon: <AlertTriangle className="w-4 h-4 mr-1 text-amber-700" />,
        label: 'Low Stock',
    },
    out_of_stock: {
        gradient: 'from-rose-100 to-rose-300 text-rose-900',
        icon: <AlertCircle className="w-4 h-4 mr-1 text-rose-700" />,
        label: 'Out of Stock',
    },
};

export default function Index({ inventory, warehouses, filters }) {
                    ...
                    <div className="bg-white/80 backdrop-blur-sm overflow-x-auto shadow-xl rounded-lg">
                        <div className="p-6 min-w-[600px]">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-6">
                                <h1 className="text-2xl font-bold text-[#54483A]">All Inventory</h1>
                                            ...
                                            <tr key={item.id} className="hover:bg-[#F0EBE3] transition-colors duration-200">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className={`
                                                        ...
                                                        bg-gradient-to-r ${statusStyles[item.quantity === 0 ? 'out_of_stock' : item.quantity <= item.product.reorder_point ? 'low_stock' : 'in_stock']?.gradient || 'from-gray-100 to-gray-300 text-gray-800'}
                                                        border border-white/40
                                                        backdrop-blur-sm bg-opacity-80
                                                        transition-all duration-200
                                                    `}>
                                                        {statusStyles[item.quantity === 0 ? 'out_of_stock' : item.quantity <= item.product.reorder_point ? 'low_stock' : 'in_stock']?.icon}
                                                        {statusStyles[item.quantity === 0 ? 'out_of_stock' : item.quantity <= item.product.reorder_point ? 'low_stock' : 'in_stock']?.label}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-[#8B7355]">
